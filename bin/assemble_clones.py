...
def parse_args():
    ...
    for sp in [sp1, sp2]:
        ...
        sp.add_argument('-fa', '--fasta', dest='fasta', type=str, 
                        help='Input fasta file from deduplication.py')
        sp.add_argument('--plot', action='store_true', 
                        help='Plot V and J scores with cutoff')
        sp.add_argument('--full_name', action='store_true', 
                        help='Retain full name of first V and J genes')
        sp.add_argument('--minimal', action='store_true', 
                        help='Work with and output only a minimal table')
        sp.add_argument('--threads', dest='nthreads', default=1, type=int, 
        ...
        sp.add_argument('--call_dj', dest='call_dj', action='store_true', 
                        help='Call DJ recombination, else only VDJ will be called')

    sp2.add_argument('--json_path', dest='json_path', type=str, 
                    help='JSON file produced during precleaning that contains identity of assembled and unassembled reads')
    sp3.add_argument('-tsv', '--tsv_files', dest='tsv_files', type=str, metavar='x.tsv', nargs='+', 
                    help='Input tsv files from previous run of assemble_clones.py')

    for sp in [sp1, sp2, sp3]:
        sp.add_argument('--threshold', dest='thres', type=int, default=1, 
                        help='Number of differences allowed between CDR3 sequences [1]')
        sp.add_argument('--only_v', action='store_true', 
                        help='Use only V idenity and CDR3 for clone assembly')
        sp.add_argument('--out', dest='out_dir', type=str, 
                        help='Output directory, default: creates Deduplicated in main directory')

    sp2.add_argument('-v', '--v_end', dest='v_fastq', type=str, 
                    help='V end fastq or fasta file')
...
def adj_list_adjacency(umis, threshold=1):
    ''' identify all umis within the hamming distance threshold
    
    Args:
        umis(list): A list of UMI strings
        threshold(int): hamming distance allowed less than or equal

    Return:
        Dictionary with K umi1 V umi2
    '''
...
def read_changeo_out(tab_file, out, prefix, fasta, v_fastq=None, plot=False, retain_nam=False,
    ...
    Args:
        ...
        tab_file(str): MakeDb.py output file (input file)
        out(str): Output directory
        prefix(str): Prefix of output files
        fasta(str): Fasta file from deduplication.py
    '''
...
def assemble_colonotype(pd_data_frame, bundles, threshold):
    ...
    for bundle in tqdm(bundles.values()):
        ...
        adj_list = adj_list_adjacency(list(umis), threshold)
