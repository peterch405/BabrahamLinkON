...
# from general import species, fastqHolder, fastq_parse
...
class deduplicate:
    ...
    def remove_duplicates(self, spe='mmu', barcode_1='GACTCGT', barcode_2='CTGCTCCT', verbose=True, write=True, plot=True, cores=1, dedup_full=False):
        ...
        #All V basenames should be same (one sample per folder)
        # prefix_set = set()
        # for v_file in self.V_files:
        #     prefix_set.add(re.split('(_V)', os.path.basename(v_file))[0])
        #
        # assert len(prefix_set) == 1, 'Mutiple samples present in folder!'
        #
        #
        # with open(self.tmp_dir + '/' + prefix_set[0] + '_V_merged.fastq, 'w') as out_v:
        #     for to_merge in self.V_files:
        #
        #         if verbose:
        #             print('Merging:', v_file)
        #
        #         with file_open(to_merge) as Vr:
    ...
    def extract_original(self, merge=True):
        ...
        #put all files into dict (memory intensive)
        # j_dict = general.fastqHolder()
        # v_dict = general.fastqHolder()
        #
        # for f in self.V_files:
        #     with open(f, 'r') as v_file:
        #
        #         lines = v_file.read().splitlines()
        #
        ...
        #             thrd = item[2]
        #             qual = item[3]
        #             #assumes all files have unique reads (no duplicate qnames between files)
        #             v_dict.add_to_fastq(title.split(' ')[0], title, seq, thrd, qual)
        #
        # for f in self.J_files:
        #     with open(f, 'r') as j_file:
        #
        #         lines = j_file.read().splitlines()
        #
        #         for item in general.fastq_parse(lines):
        #             title = item[0]
        #             seq = item[1]
        ...
        #             qual = item[3]
        #             #assumes all files have unique reads (no duplicate qnames between files)
        #             j_dict.add_to_fastq(title.split(' ')[0], title, seq, thrd, qual)
        ...
        for name in dir_files:
            ...
            with open(out_j, 'w') as j_out:
                ...
                #     fq_lines = v_dict.fastq_split[item]
                #     v_file_out.write(fq_lines[0][0] + '\n' + fq_lines[0][1] + '\n' + fq_lines[0][2] + '\n' + fq_lines[0][3] + '\n')
                # except KeyError:
                #     print('Key not in V dictionary')
                #
                # try:
                #     fq_lines = j_dict.fastq_split[item]
                #     j_file_out.write(fq_lines[0][0] + '\n' + fq_lines[0][1] + '\n' + fq_lines[0][2] + '\n' + fq_lines[0][3] + '\n')
                # except KeyError:
                #     print('Key not in J dictionary')
...
def parse_args():
    ...
    parser.add_argument('--input_dir', dest='in_dir', type=str, required=True, help='Input directory (created by preclean)')
...
def main():
    ...
    dedup = deduplicate(opts.in_dir) #initialise class
